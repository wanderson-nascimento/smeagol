...
const registerNetworkIpc = (mainWindow) => {
  ...
  ipcMain.handle(
    'renderer:run-collection-folder', async (event, folder, collection, environment, runtimeVariables, recursive, delay, tags, selectedRequestUids, iterationRows = null) => {
      const collectionUid = collection.uid;
      const collectionPath = collection.pathname;
      ...
      try {
        ...
        const iterationDataRows = Array.isArray(iterationRows) && iterationRows.length ? iterationRows : [{}];

        for (let iterationIndex = 0; iterationIndex < iterationDataRows.length; iterationIndex++) {
          const iterationVariables = iterationDataRows[iterationIndex];

          if (iterationDataRows.length > 1) {
            mainWindow.webContents.send('main:run-folder-event', {
          ...
          while (currentRequestIndex < folderRequests.length) {
            ...
            request.iterationVariables = iterationVariables;
            request.iterationIndex = iterationIndex;
            request.totalIterations = iterationDataRows.length;
          ...
          if (abortController.signal.aborted || stopRunnerExecution) {
            break;
          }
        }
        ...
        mainWindow.webContents.send('main:run-folder-event', {
          ...
          ...(runnerTerminated ? { statusText: 'collection run was terminated!' } : {})
        });
      } catch (error) {
